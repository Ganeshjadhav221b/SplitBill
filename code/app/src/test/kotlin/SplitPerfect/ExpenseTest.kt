...
class ExpenseTest() {
    ...
    fun checkExpenseCreation()
    {
        ...
        val expense: Expense? = expenseService.addExpense(Expense(1,100,"Cab", LocalDateTime.now(),user1,bill1))
        assertEquals(expense, expense?.Id?.let { expenseService.findExpense(it) })
    }

    @Test
    fun checkExpenseDeletion()
    {
        val user1 =  userService.addUser(User(1,"john","john@gmail.com",100))
        val group1 =  groupService.addGroup(Groups(1,"Hunters","we party day and night",null))
        val bill1 = billService.addBill(Bill(1,110,0,0,"hotel ram",null, LocalDateTime.now(),group1))
        val expense: Expense? = expenseService.addExpense(Expense(1,100,"Cab", LocalDateTime.now(),user1,bill1))
        expense?.let { expenseService.deleteExpense(it.Id) }
        if (expense != null) {
            assertEquals(null,expenseService.findExpense(expense.Id))
        }
